...
ordinals of the field (`global_ordinals`). The latter is the is faster, especially for fields with many unique
values. However it can slower if only a few documents match, when for example a terms aggregator is nested in another
