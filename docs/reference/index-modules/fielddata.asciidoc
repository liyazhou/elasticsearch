...
Global ordinals is a data-structure on top of field data, that maintains a
...
Global ordinals can be beneficial in search features such as the terms aggregator
...
on a shard. So per field and per shard there is one global ordinal instance.
Unlike field data global ordinals need to rebuild in its entirety, once new segments
become visible during a refresh. Building or rebuilding global ordinals is relatively
cheap, since its source, field data is in memory all ready.
